...
MLFLOW_EXPERIMENT_NAME = "insurance_regression"
...
with DAG(
    # dag_id="training_pipeline_6",
    dag_id="training_pipeline_insurance_rf_gb_lr_ab_1",
    default_args=default_args,
    description="Training DAG for medical insurance regression model",
    ...
    train_model = DataprocCreatePysparkJobOperator(
        task_id="train",
        main_python_file_uri=f"{S3_SRC_BUCKET}/model_train_insurance.py",
        connection_id=YC_SA_CONNECTION.conn_id,
        dag=dag,
        args=[
            # "--input", f"{S3_INPUT_DATA_BUCKET}/train.csv",
            "--input", f"{S3_INPUT_DATA_BUCKET}/output_data/",
            "--output", f"{S3_OUTPUT_MODEL_BUCKET}/insurance_model_{datetime.now().strftime('%Y%m%d')}",
